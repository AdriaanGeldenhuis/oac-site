...
if ($commentId <= 0) {
    ...
    echo json_encode(['ok' => false, 'error' => 'missing_comment_id']);
    exit;
}
...
try {
    // Only users with access to the comment's room may see who reacted
    $roomStmt = $pdo->prepare("
        SELECT r.* FROM rooms r
        JOIN posts p ON p.room_id = r.id
        JOIN comments c ON c.post_id = p.id
        WHERE c.id = ? LIMIT 1
    ");
    $roomStmt->execute([$commentId]);
    ...
    error_log('Comment reactions list error: ' . $e->getMessage());
    http_response_code(500);
